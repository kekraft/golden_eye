...
const int stop_motor_speed = 0;
const int motor_speed = 127;
const int lightning_motor_speed = 175;

bool lightning_mode = false;
...
void run_lightning_motor(){
  analogWrite(motor_pin, lightning_motor_speed);
}
...
void stop_motor(){
  ...
  analogWrite(motor_pin, stop_motor_speed);
...
void lightning_cb(const std_msgs::UInt16& cmd_msg){
   
    state_msg.data = "Received lighting message";
    pub_state.publish(&state_msg);

    // check cmd_msg
    
    int msg_int = cmd_msg.data;
    if (msg_int == 1) {
      // run motor at higher speed
      lightning_mode = true;
      
    } else {
      // run the motor until stopped by switch
      lightning_mode = false;
    }
}

ros::Subscriber<std_msgs::UInt16> lightning_sub("/loader/lightning_cmd", lightning_cb);
